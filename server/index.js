...
app.put('/update', (req,res) =>{
    ...
    db.query('UPDATE inventory SET name=COALESCE(NULLIF(?, ""), name), description=COALESCE(NULLIF(?, ""), description), stock = COALESCE(NULLIF(?, ""), stock) WHERE id = ?', [name, description, stock, id], (err, result) =>{
        if(err){
            console.log(err);
        } else {
            res.send(result);
